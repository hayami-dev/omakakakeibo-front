...
1. ログイン画面（`/auth/login`）にある「新規アカウント作成」ボタンを押して`/auth/register`へアクセスします。

2. メールアドレスを入力して「認証メールを送信する」ボタンを押してください。
   ※開発段階ではメールの送信はしないため、半角英数字、`@`、`.`を含むメールアドレスの形式であればOKです。
   例：`aaa@aaa.aaa`など

3. 開発段階では、バックエンドのコンソールへトークンを含んだURLが表示されるので、ブラウザ上でアクセスしてください。
   有効期限は30分です。

4. パスワードを半角英数字、記号を含めて8～12文字で入力してください。
   例：`password@1`など

5. 目標金額を入力して「目標金額を設定する」ボタンを押してください。（スキップボタンを押しても大丈夫です。）

6. 新規アカウントが作成されました。
   「ホームへすすむ」ボタンを押して、ホーム画面へ移動できます。
